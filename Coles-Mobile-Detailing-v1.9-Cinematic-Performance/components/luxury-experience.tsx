...
export function LuxuryExperience() {
  ...
  return <main>
    ...
    <AnimatePresence>{menu && <motion.div className="mobile-menu" role="dialog" aria-modal="true" aria-label="Site navigation" initial={{ clipPath: "inset(0 0 100% 0)" }} animate={{ clipPath: "inset(0 0 0% 0)" }} exit={{ clipPath: "inset(0 0 100% 0)" }} transition={{ duration: .65, ease: [0.16,1,0.3,1] }}><div className="mobile-menu-top"><div className="mobile-menu-brand"><Image src="/images/brand/coles-mobile-detailing-badge-small.webp" alt="Cole's Mobile Detailing logo" width={54} height={54} /><span>COLE&apos;S / MOBILE DETAILING</span></div><button onClick={() => setMenu(false)} aria-label="Close navigation">Close</button></div><nav>{nav.map((item,index) => <motion.a initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .12 + index*.06 }} onClick={() => setMenu(false)} key={item} href={`#${item.toLowerCase().replaceAll(" ", "-").replace("&", "and")}`}><small>0{index+1}</small><span>{item}</span><b>↗</b></motion.a>)}</nav><div className="mobile-menu-actions"><a href="tel:+18126295544">Call 812-629-5544</a><a href="sms:+18126295544">Text to book</a></div><p>Newburgh · Evansville · Southern Indiana</p></motion.div>}</AnimatePresence>
