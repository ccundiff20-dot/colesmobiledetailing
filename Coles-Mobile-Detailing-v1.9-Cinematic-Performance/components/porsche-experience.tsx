...
export function PorscheExperience() {
  ...
  useEffect(() => {
    if (!nearViewport || mobile || enabled) return;
    const id = window.setTimeout(() => setEnabled(true), 650);
  ...
  return <section ref={section} className="porsche-experience" aria-label="Interactive Porsche detailing experience" onPointerMove={(event) => {
    ...
    <div className="porsche-sticky">
      ...
      {enabled ? <div className="porsche-canvas">
        <Canvas dpr={mobile ? 0.85 : [0.9, 1.05]} frameloop={nearViewport ? "always" : "demand"} camera={{ position: [3.8, 0.78, 5.05], fov: 22, near: 0.05, far: 40 }} gl={{ antialias: !mobile, alpha: true, powerPreference: "high-performance" }} performance={{ min: 0.55, max: 1 }}>
          <CameraRig />
