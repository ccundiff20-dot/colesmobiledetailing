...
The project files must be placed directly at the connected GitHub repository root. After uploading, GitHub's top level must show `app`, `components`, `lib`, `public`, `package.json`, and `netlify.toml`. Do not upload the enclosing `Coles-Mobile-Detailing-V6.2-LEGAL-AI` folder as a subfolder; Netlify will otherwise build the older root project and report that it cannot find an `app` or `pages` directory.
